...
class Detector(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.img_height_orig = 720
        self.img_height = 480.0 #720.0
        self.img_width_orig = 1280.0
        self.img_width = 640.0 #1280.0
    ...
    def out_to_bbs(
        ...
        for o in out:
            ...
            for bb_index in bb_indices:
                ...
                # Added: Determine category index
                bb_category_idx = torch.argmax(o[5:, bb_index[0], bb_index[1]]) # get the index of the highest category score
    ...
    def anns_to_target_batch(self, anns: Tuple[dict]) -> Tuple[torch.Tensor]:
        ...
        for i, ann in enumerate(anns):
            cats = ann["labels"]
            for c_idx, box in enumerate(ann["boxes"]):
                
                # Scale bounding box so it fits new image size
                #print(f"BOX1: {box}")
                """
                x_min, y_min, x_max, y_max = box
                x_scale_factor = self.img_width / self.img_width_orig
                y_scale_factor = self.img_height / self.img_height_orig
                x_min_scaled = x_min * x_scale_factor
                x_max_scaled = x_max * x_scale_factor
                y_min_scaled = y_min * y_scale_factor
                y_max_scaled = y_max * y_scale_factor
                box = torch.tensor([x_min_scaled, y_min_scaled, x_max_scaled, y_max_scaled])
                ann["boxes"][c_idx] = box
                #print(f"BOX2: {box}")
                """
                ...
                # Mark the category in target with 1 (instead of 0)
                target[i,5+c, y_ind, x_ind] = 1
                c_idx += 1 

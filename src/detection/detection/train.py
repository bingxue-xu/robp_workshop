...
from torchvision.transforms import transforms, functional #added
...
#category_dict = {0:"c0", 1:"c1", 2:"c2", 3:"c3", 4:"c4", 5:"c5", 6:"c6", 7:"c7"}
category_dict = {0:"Binky", 1:"Hugo", 2:"Slush", 3:"Muddles", 4:"Kiki", 5:"Oakie", 6:"Cube", 7:"Ball"}
def compute_loss(
    ...
    #loss_output = loss(predict_class_reshaped,target_class_reshaped)
    # compute loss
    ...
    # Added: Calculates loss comparing categories
    loss = nn.CrossEntropyLoss() 
    loss_output_cat = loss(prediction_batch[:, 5:, :, :], target_batch[:, 5:, :, :],
    )

    #loss_output_cat = loss(prediction_batch[pos_indices[0], 5:, pos_indices[1], pos_indices[2]],
    ...
    #print(f"pos:{pos_mse}")
    #print(f"neg:{neg_mse}")
    print(f"Cat loss:{loss_output_cat}")
    return reg_mse, pos_mse, neg_mse, loss_output_cat
...
def train(device: str = "cpu") -> None:
    ...
    """
    wandb.init(project="classification_test3")
    ...
    # Added: resize images
    #resize_transform = v2.Resize((480, 640))

    #v2.Resize(size=(480, 640)),

    # Defines input transformations for images
    # 1. Converts to tensor 2. Pixel values to [0,1] 3. Normalizes
    ...
    aug_transforms = v2.Compose(
        [
            ...
            v2.RandomResizedCrop((480, 640), scale=(0.5, 1.0)),
        ]
    )

 
    # Prepares training dataset (json file contains info about bboxes and categories)    
    dataset = CocoDetection(
        root="./DD2419_data/training",
        annFile="./DD2419_data/annotations/merged_training.json",
        transforms=input_transforms,
    )    
    dataset = wrap_dataset_for_transforms_v2(dataset)
    
    aug_dataset = CocoDetection(
        root="./DD2419_data/training",
        annFile="./DD2419_data/annotations/merged_training.json",
        transforms=v2.Compose([input_transforms, aug_transforms]),
    )    
    aug_dataset = wrap_dataset_for_transforms_v2(aug_dataset)
    # Prepares validation dataset
    val_dataset = CocoDetection(
        root="./DD2419_data/validation",
        annFile="./DD2419_data/annotations/merged_validation.json",
    ...
    if test_images:
        ...
        img1 = test_image_batch[0]
        save_image(img1, 'img1.png')
        print("Test image sizes:", [img.size() for img in test_image_batch])
    print("Training started...")

    current_iteration = 1
    while current_iteration <= NUM_ITERATIONS:
        for images, anns in dataloader:
            #print("Original image sizes:", [img.shape for img in images])
            ...
            img1 = img_batch[0]
            save_image(img1, 'img2.png')
            # Print size of transformed images
            #print("Transformed image sizes:", [img_batch[i].shape for i in range(len(img_batch))])


            # run network
            out = detector(img_batch)

            reg_mse, pos_mse, neg_mse, loss_output_cat = compute_loss(out, target_batch)
            loss = WEIGHT_POS * pos_mse + WEIGHT_REG * reg_mse + WEIGHT_NEG * neg_mse + WEIGHT_CAT * loss_output_cat
            ...
            if current_iteration % VALIDATION_ITERATION == 1 and show_test_images:
                ...
                with torch.no_grad():
                    ...
                    for i, test_image in enumerate(test_images):
                        ...
                        print(f"Test image {i} size: {test_image.size}")
                        # add bounding boxes
                        ...
                        save_image(result_image, 'test2.png')
                        print(f"Result test image {i} size: {result_image.size()}")
                        plt.close()
                detector.train()
                    ...
                    coco_pred.dataset["annotations"].append(
                        {
                            ...
                            "category_id": img_bb["category"],  # Done TODO replace with predicted category id
                            "score": img_bb["score"],
                            "image_id": image_id,
                        }
                    )
        ...
        wandb.log(
            {
                ...
                "val loss cat": (total_loss_cat / count),
